...
namespace Backend_OddityVR.Prospe
{
    public class ProspeRepo
    {
        ...
        public void CreateNewProspe(Prospe prospe)
        {
            string query =
                "INSERT INTO Prospe " +
                "(DateMessage, Name, Email, Phone, Subject, Message) " +
        ...
        private List<Prospe> ToModel(SqlDataReader reader)
        {
            ...
            while (reader.Read())
            {
                listCompanies.Add(new Prospe()
                {
                    ...
                    DateMessage = DateTime.Parse(reader["DateMessage"].ToString()),
                    ReadDate = DateTime.TryParse(reader["ReadDate"].ToString(), out DateTime dateResult) ? dateResult : null,
